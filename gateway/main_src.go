...
func (m *MainSrc) Write() error {
	...
	for _, schema := range m.schemas {
		...
		res = append(res, fmt.Sprintf(`	r.GET("/%ss", toGin(%sController.List))`, schema.LowerSchemaName(), schema.LowerSchemaName()))
		res = append(res, fmt.Sprintf(`	r.GET("/%ss/:id", toGin(%sController.Get))`, schema.LowerSchemaName(), schema.LowerSchemaName()))
		res = append(res, fmt.Sprintf(`	r.POST("/%ss", toGin(%sController.Post))`, schema.LowerSchemaName(), schema.LowerSchemaName()))
		res = append(res, fmt.Sprintf(`	r.PUT("/%ss/:id", toGin(%sController.Put))`, schema.LowerSchemaName(), schema.LowerSchemaName()))
		res = append(res, fmt.Sprintf(`	r.DELETE("/%ss/:id", toGin(%sController.Delete))`, schema.LowerSchemaName(), schema.LowerSchemaName()))
	}
	res = append(res, ``)
